// LRU cache design (Least Recently Used)
// we keep most recent item in cache, and remove the least recent item
// if the current element teken is not in cache, we call it a miss and put that element in the cache
// if the current item is a hit, we shift it to recent position
// if the cache size is full, we remove the least recent;y used item

// simple sol with array, but hit and miss takes O(n) TC
// we need to have a DS with fastest search and insert
// we can use hash to do so
// but to maintain ordering , we also use DLL
...
class LRUCache {
	...
	// deleting a node from anwhere in DLL
